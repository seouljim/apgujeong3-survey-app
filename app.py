...
def get_submission_by_key(unit_key):
    sb = get_supabase()
    res = sb.table("submissions").select("*").eq("unit_key", unit_key).limit(1).execute()
...
def insert_submission(data):
    ...
    sb.table("submissions").insert(payload).execute()
...
def update_submission(existing, data):
    ...
    for k in ["name", "existing_dong", "existing_ho", "existing_size", "unit_key"]:
        payload.pop(k, None)
    sb.table("submissions").update(payload).eq("id", existing["id"]).execute()
    sb.table("edit_logs").insert({
...
def check_unique_rank(values, label):
    values = [v for v in values if v and v != "선택 안함"]
    if len(values) != len(set(values)):
        return False, f"{label} 순위는 중복 없이 선택해 주세요."
    return True, ""
...
def render_header_notice():
    st.markdown(
        """
        본 조사는 조합원들의 희망 평형·평면·커뮤니티·조경·준주거용지 특화계획 수요를 파악하기 위한 온라인 의견수렴 조사입니다.
...
def render_step1():
    ...
    current_size_other = ""
    if existing_size == "기타":
        current_size_other = st.text_input("현재 평형 직접 입력", placeholder="예: 48평")

    if st.button("다음으로", type="primary", use_container_width=True):
        if not consent:
            st.error("개인정보 수집·이용에 동의해야 다음 단계로 진행할 수 있습니다.")
            return
        if not all([name.strip(), existing_dong.strip(), existing_ho.strip(), existing_size]):
            st.error("성명, 동, 호수, 현재 평형을 모두 입력해 주세요.")
            return
        if existing_size == "기타" and not current_size_other.strip():
            st.error("현재 평형을 직접 입력해 주세요.")
            return

        size_final = current_size_other.strip() if existing_size == "기타" else existing_size
        unit_key = make_unit_key(name, existing_dong, existing_ho)
        st.session_state.verified = {
            "name": name.strip(),
            "existing_dong": existing_dong.strip(),
            "existing_ho": existing_ho.strip(),
            "existing_size": size_final,
            "unit_key": unit_key,
        }
...
def rank_selectors(title, options, prefix, count=5, allow_skip=False):
    ...
    for i in range(count):
        with cols[i]:
            val = st.selectbox(f"{i+1}순위", select_options, key=f"{prefix}_{i}")
            selected.append(val)
    return selected
...
def render_step2():
    ...
    with st.expander("Q01. 입주 가족인원·희망 평면·방 개수", expanded=True):
        ...
        family_count = None
        if family_plan == "입주 계획 있음":
            family_count = st.number_input("입주 시 함께 거주하실 가족인원 수", min_value=1, max_value=20, value=2, step=1)
    ...
    with st.expander("Q03. 평형 외 가장 중요하게 생각하는 요소", expanded=True):
        factor_rank = rank_selectors("중요한 순서대로 선택해 주세요.", IMPORTANT_FACTORS, "factor", count=5)
        ok, msg = check_unique_rank(factor_rank, "중요 요소")
        if not ok:
            st.warning(msg)

    with st.expander("Q04. 희망 평형", expanded=True):
        st.caption("원하는 평형 1곳을 선택해 주세요.")
        hope_size = st.radio("희망 평형", HOPE_SIZE_OPTIONS, horizontal=False)

    with st.expander("Q05. 희망 세대당 주차대수", expanded=False):
        parking = st.radio("희망하시는 세대당 주차대수", PARKING_OPTIONS, horizontal=True)
        parking_other = ""
        if parking == "기타":
            parking_other = st.text_input("주차대수 기타 의견", placeholder="예: 3.5대 이상")

    with st.expander("Q06. 희망 커뮤니티 시설", expanded=False):
        community_rank = rank_selectors("선호하는 순서대로 선택해 주세요.", COMMUNITY_OPTIONS, "community", count=5, allow_skip=True)
        community_other = st.text_input("커뮤니티 기타 의견", placeholder="예: 의료상담실, 소규모 강의실 등")
        ok2, msg2 = check_unique_rank(community_rank, "커뮤니티")
        if not ok2:
            st.warning(msg2)

    with st.expander("Q07. 희망 조경시설", expanded=False):
        landscape_rank = rank_selectors("선호하는 순서대로 선택해 주세요.", LANDSCAPE_OPTIONS, "landscape", count=5, allow_skip=True)
        landscape_other = st.text_input("조경시설 기타 의견", placeholder="예: 반려견 산책로 등")
        ok3, msg3 = check_unique_rank(landscape_rank, "조경시설")
        if not ok3:
            st.warning(msg3)
    ...
    with st.expander("Q08. 준주거용지 특화계획시설", expanded=False):
        ...
        semi_residential_other = ""
        if "기타" in semi_residential:
            semi_residential_other = st.text_input("준주거용지 기타 의견", placeholder="희망 시설을 입력해 주세요.")
    ...
    with c2:
        if st.button("제출 내용 확인", type="primary", use_container_width=True):
            ...
            st.session_state.survey = {
                ...
                "factor_rank_1": factor_rank[0],
                "factor_rank_2": factor_rank[1],
                "factor_rank_3": factor_rank[2],
                "factor_rank_4": factor_rank[3],
                "factor_rank_5": factor_rank[4],
                "hope_size": hope_size,
                "parking": parking,
                "parking_other": parking_other.strip(),
                "community_rank_1": community_rank[0],
                "community_rank_2": community_rank[1],
                "community_rank_3": community_rank[2],
                "community_rank_4": community_rank[3],
                "community_rank_5": community_rank[4],
                "community_other": community_other.strip(),
                "landscape_rank_1": landscape_rank[0],
                "landscape_rank_2": landscape_rank[1],
                "landscape_rank_3": landscape_rank[2],
                "landscape_rank_4": landscape_rank[3],
                "landscape_rank_5": landscape_rank[4],
                "landscape_other": landscape_other.strip(),
                "semi_residential": ", ".join(semi_residential),
                "semi_residential_other": semi_residential_other.strip(),
                "memo": memo.strip(),
            }
...
def render_step3():
    ...
    st.write(f"- 성명: {v['name']}")
    st.write(f"- 소유 동·호수: {v['existing_dong']} {v['existing_ho']}")
    st.write(f"- 현재 평형: {v['existing_size']}")

    st.markdown("### 설문 응답 요약")
    st.write(f"- 입주 계획: {s.get('family_plan')}")
    if s.get("family_count"):
        st.write(f"- 가족인원 수: {s.get('family_count')}명")
    st.write(f"- 희망 평면: {s.get('plan_preference')}")
    st.write(f"- 방 개수: {s.get('room_count')}")
    st.write(f"- 세대구분형 옵션: {s.get('household_split')}")
    st.write(f"- 희망 평형: **{s.get('hope_size')}**")
    ...
    c1, c2 = st.columns(2)
    with c1:
        if st.button("설문 수정하기", use_container_width=True):
            go_step(2)
    with c2:
        if st.button("최종 제출", type="primary", use_container_width=True):
            ...
            try:
                if existing is None:
                    ...
                    success, msg = update_submission(existing, data)
                    if success:
                        st.success(msg)
                    else:
                        st.error(msg)
...
def render_check():
    ...
    name = st.text_input("성명", key="check_name")
    c1, c2 = st.columns(2)
    with c1:
        dong = st.text_input("소유 동", key="check_dong")
    with c2:
        ho = st.text_input("소유 호수", key="check_ho")

    if st.button("확인하기", key="check_btn", use_container_width=True):
        if not all([name.strip(), dong.strip(), ho.strip()]):
            st.error("성명, 동, 호수를 입력해 주세요.")
        else:
            key = make_unit_key(name, dong, ho)
            existing = get_submission_by_key(key)
            if not existing:
                st.warning("제출 내역을 찾을 수 없습니다.")
            else:
                st.success("제출 내역이 확인되었습니다.")
                st.write(f"성명: {existing.get('name')}")
                st.write(f"소유 동·호수: {existing.get('existing_dong')} {existing.get('existing_ho')}")
                st.write(f"현재 평형: {existing.get('existing_size')}")
                st.write(f"희망 평형: {existing.get('hope_size')}")
                st.write(f"수정 횟수: {existing.get('edit_count', 0)}회 / 1회 가능")
...
def counts_table(df, column, options=None):
    ...
    if options:
        vc = vc.reindex(options + ["미응답"], fill_value=0)
    out = vc.rename_axis("항목").reset_index(name="응답 수")
    out["비율"] = (out["응답 수"] / len(df) * 100).round(1).astype(str) + "%"
    return out
...
def render_admin():
    ...
    m1.metric("전체 참여자 수", f"{len(df):,}명")
    m2.metric("현재 평형 수", f"{df['existing_size'].nunique():,}개")
    m3.metric("희망 평형 수", f"{df['hope_size'].nunique():,}개" if "hope_size" in df.columns else "0개")
    m4.metric("수정 제출", f"{(pd.to_numeric(df.get('edit_count', 0), errors='coerce').fillna(0) > 0).sum():,}건")

    st.divider()

    st.subheader("희망 평형")
    hope_size_table = counts_table(df, "hope_size", HOPE_SIZE_OPTIONS)
    st.dataframe(hope_size_table, use_container_width=True)
    if not hope_size_table.empty:
        st.bar_chart(hope_size_table.set_index("항목")[["응답 수"]])

    st.subheader("현재 평형 × 희망 평형")
    if "existing_size" in df.columns and "hope_size" in df.columns:
        pivot = pd.crosstab(df["existing_size"], df["hope_size"])
        st.dataframe(pivot, use_container_width=True)

    st.subheader("Q03 중요 요소 1순위")
    st.dataframe(counts_table(df, "factor_rank_1", IMPORTANT_FACTORS), use_container_width=True)

    st.subheader("Q05 주차대수")
    st.dataframe(counts_table(df, "parking", PARKING_OPTIONS), use_container_width=True)

    st.subheader("Q06 커뮤니티 1순위")
    st.dataframe(counts_table(df, "community_rank_1", COMMUNITY_OPTIONS), use_container_width=True)

    st.subheader("Q07 조경 1순위")
    st.dataframe(counts_table(df, "landscape_rank_1", LANDSCAPE_OPTIONS), use_container_width=True)

    st.subheader("Q08 준주거용지 특화계획")
    if "semi_residential" in df.columns:
        exploded = df["semi_residential"].fillna("").str.split(", ").explode()
        exploded = exploded[exploded != ""]
        semi_df = exploded.value_counts().rename_axis("항목").reset_index(name="응답 수")
        st.dataframe(semi_df, use_container_width=True)

    st.divider()
    anonymized = df.drop(columns=["id", "name", "unit_key"], errors="ignore")
    st.download_button(
        "개인정보 제외 응답 CSV 다운로드",
        data=safe_download_csv(anonymized),
        file_name="apgujeong3_survey_anonymized.csv",
        mime="text/csv",
        use_container_width=True,
    )

    with st.expander("원본 응답 데이터 보기 / 다운로드"):
        st.warning("원본 데이터에는 개인정보가 포함됩니다. 접근 권한을 제한해 주세요.")
        st.dataframe(df, use_container_width=True)
        st.download_button(
            "원본 응답 CSV 다운로드",
            data=safe_download_csv(df),
            file_name="apgujeong3_survey_raw.csv",
            mime="text/csv",
            use_container_width=True,
        )
